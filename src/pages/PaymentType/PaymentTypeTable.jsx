...
const DataTableComponent = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        const response = await fetch(`${apiBaseUrl}/test/api/payment-type`, {
          method: "GET",
          headers: {
        ...
        if (dataTableRef.current) {
          ...
          dataTableRef.current.rows.add(
            data.map((paymentType) => [
                paymentType.id,
                paymentType.name,
              `<button class="btn btn-success btn-update col-12" data-id="${paymentType.id}">Update</button>`
            ])
          );
        ...
        $(".btn-update").on("click", function () {
          ...
          navigate(`/update-payment-type/${paymentTypeId}`);
        });
